...
from models.links import (
    Link, ShortLink, Links, ShortLinks, ClientData, ClickData,
    ClickDataShortResponse, ClickDataFullResponse, DbStatus)
...
@router.post("/shorten", name="Получить список сокращённых вариантов переданных url", tags=TAGS, description="Получить список сокращённых вариантов переданных URL")
async def post_link(
        links: Links,
        link_service: BaseLinkService = Depends(get_link_service),
):
...
@router.get("/{shorten-url-id}", name="Получить оригинальный url", tags=TAGS, description="Вернуть оригинальный URL")
async def get_link(
        shorten_url_id: str,
        request: Request,
        link_service: BaseLinkService = Depends(get_link_service),
...
@router.get("/ping", name="Статус доступности бд", tags=TAGS, description="Возвращает информацию о статусе доступности БД")
async def get_ping(
        link_service: BaseLinkService = Depends(get_link_service),
) -> DbStatus:
